...
public class Python3Normalizer implements Normalizer {
     ...
     * @param modes Configuration specifications for which category of symbols are to be normalised
     */
    @Override
    public void configure(NormalizerMode modes) {
        this.python3modes = (Python3NormalizerMode) modes;

        normalizeSettings = ImmutableMap.<String, Boolean>builder()
                .put(NORMALIZED_TOKEN_KEYWORD, python3modes.isKeywordToBeNormalised())
                .put(NORMALIZED_TOKEN_VALUE, python3modes.isValueToBeNormalised())
